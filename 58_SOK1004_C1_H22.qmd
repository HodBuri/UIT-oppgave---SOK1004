...
# Create the tibble data frame from the dataset in df
tibbel_df <- as_tibble(df$dataset)

#convert "tibbel_data$Tid" column values from <chr> to <int>
tibbel_df <- tibbel_df %>% mutate(Tid = as.integer(Tid))
...
# New code (needed to change "df" to "tibbel_df")
  # Needed to do this change, I assume you might have expected med to save the tibble as "df"
  # But I am used to use VAR names that I find to be clear and specify what it actually is.
  # You know code should be readable without comments
  # Or you might have wanted me to change the column names of "df" in "Oppgave Ia: Rydd i data".
...
# Litten endring, fjernet et library jeg testet ut
library(scales)
...
tibbel_df %>%
  ...
  scale_x_continuous(n.breaks=21,expand = expansion(mult = c(0, 0)))+
  ggtitle("Norge Nominal BNP vs Real BNP",subtitle = "Real BNP er ankret til 2015") +
  xlab("Year (2000-2021)") + ylab("BNP NOK")+
  labs(color = "BNP type") +
...
Skriv en tydelig forklaring og tolkning av figuren.

Denne figuren viser forskellen mellom løpende og fast BNP mellom 2000 of 2021, der fast BNP er ankret til 2015.

Begge grafene viser at BNP har vekst i de siste 21 årene

Forklar forskjellen mellom BNP i løpende og faste priser.

Løpende BNP er når datan er lik orginal prisene får den tiden, og det hjelper med å se økonomiens vekst som en hellhet

Fast BNP er når man ankerer prisene til ett år, slik at man kan fjærne inflasjon faktoren. Det vil gi rein production value

Løpende hvis du har løst å se på veksten av økonomien

fast hvis du har lyst å fokusere på production, kjøpkraft og ting slik som det der inflasjon kan være forstyrende
...
den tar datan fra "var" og fløter det over til å bli column name, etter det så tar den datan fra "verdi" column og setter den inn der det er relevant

Fra et programerings perspektiv så kan det være similert til dette:

df = {

"NameType":\["LastName","LastName","Name","Name"\],

"Names":\["Hansen","Jenssen","Sondre","Geir"\]}

pivot_wider vil da ta items fra NameType key, og lage det til nye columns. Æ kan se for meg at den arbeider på begge listene samtidig i en for loop slik at index posisjonene er lik etter de nye column navnene er laget

df_wide = {

"LastName":\["Hansen","Jenssen"\],
...
# Prosent vekst
df_wide <- df_wide %>%
  mutate(relativ_vekst_prencent_BNP = 100*DBNP/LBNP) %>%
  mutate(relativ_vekst_prencent_MEMOBNP = 100*DMEMOBNP/LMEMOBNP)

df_wide <- df_wide %>%
  relocate("relativ_vekst_prencent_BNP", .before = "MEMOBNP")
...
df_long %>%
  ...
  geom_smooth()+
  scale_y_continuous(labels = comma) + 
  scale_x_continuous(n.breaks=10,expand = expansion(mult = c(0, 0)))+
  ggtitle("Norge Løpende DBNP vs Fast DBNP",subtitle = "Fast BNP er ankret til 2015") +
  xlab("Year (2000-2021)") + ylab("BNP NOK")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5))

#heller (litt usiker ka oppgaven er ute etter)

df_long %>%
  filter(var %in% c("relativ_vekst_prencent_BNP","relativ_vekst_prencent_MEMOBNP")) %>%
  ggplot(aes(x=tid,y=verdi,color = var)) +
  geom_line()+
  geom_smooth()+
  scale_y_continuous(labels = function(y) paste(sep = "",y, "%"),n.breaks=10,expand = expansion(mult = c(0, 0))) + 
  scale_x_continuous(n.breaks=10,expand = expansion(mult = c(0, 0)))+
  ggtitle("relativ vekst prencent BNP",subtitle = "Fast BNP er ankret til 2015") +
  xlab("Year (2000-2021)") + ylab("BNP Vekst")+
  scale_colour_manual(labels = c("Nominal BNP", "Reel BNP"), values = c("#F8766D", "#00BFC4"))+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5))

```

det exsiterer negativ vekst i begge, men negativ vekst i reel BNP er ofte relatert til en høyere inflasjon en økonomisk vekst.
