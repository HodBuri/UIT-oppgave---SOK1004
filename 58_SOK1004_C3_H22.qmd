...
# løs oppgave II her
df_update <- df %>%
  filter(year>=2011,statistikkvariabel == "Konsumprisindeks (vekter)")

df_update <- subset(df_update, select = -c(month,year))
...
df_pivot <- pivot_wider(df, values_from = value,names_from = "konsumgruppe")
vekt <- subset(df_pivot, select = -c(statistikkvariabel,dato))
